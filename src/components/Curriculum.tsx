...
import { fetchSchedule, TimeSlot, DAYS, ClassSession } from "../utils/sheet";
import { X } from "lucide-react";

export default function Curriculum({ sheetId }: { sheetId: string }) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        fetchSchedule(sheetId, activeTab).then((data) => {
            if (mounted) {
                setSchedule(data);
                setLoading(false);
            }
    ...
    return (
        <section id="curriculum" className="py-16 md:py-20 bg-white text-zinc-900 overflow-hidden relative" onClick={() => setSelectedCell(null)}>
            <div className="container mx-auto px-6">
                ...
                >
                    <h2 className="text-3xl md:text-4xl font-black mb-6 italic uppercase tracking-tight">
                ...
                <div className="overflow-x-auto relative min-h-[500px] border border-zinc-200 rounded-xl shadow-lg max-h-[800px] overflow-y-auto">
                    {loading ? (
                        ...
                        <div className="text-center py-20 bg-zinc-50">
                            <p className="text-zinc-400 text-xl font-bold">시간표 데이터가 없습니다.</p>
                        </div>
                    ) : (
                        <div className="min-w-[1000px] bg-white">
                            ...
                            <div className="grid grid-cols-[120px_repeat(7,1fr)] bg-zinc-100 border-b border-zinc-200 sticky top-0 z-10 shadow-sm">
                                <div className="p-4 text-center font-black text-zinc-600 text-sm uppercase tracking-wider">시간</div>
                                {DAYS.map(day => (
                                        ...
                                        {DAYS.map(day => (
                                            <div key={day} className="p-2 flex flex-wrap gap-2 align-top content-start min-h-[80px] border-r border-zinc-50 last:border-0 relative">
                                                {slot.schedule[day] && slot.schedule[day].length > 0 ? (
                                                    slot.schedule[day].map((session: ClassSession, sIdx: number) => (
                                                        <button
                                                            key={sIdx}
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm px-4" onClick={() => setSelectedCell(null)}>
                            ...
                            >
                                ...
                                <div className="bg-zinc-50 rounded-2xl p-6 border border-zinc-100 flex items-center justify-between px-8">
                                    <span className="text-zinc-400 text-xs font-bold uppercase tracking-wider">현재 인원</span>
